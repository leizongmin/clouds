...
CloudsClient.prototype.bind = function (name, retry, onRetry) {
  ...
  var timeout = me._ns('timeout') * 1000;
  ...
  return function () {
    ...
    if (typeof onRetry !== 'function') {
      onRetry = function () {
        me._debug('call [%s]: default on retry, args=%s', name, args);
    ...
    function onCallback (err) {
      ...
      if (err && err.code === 'CLOUDS_CALL_SERVICE_TIMEOUT' && retryCounter < retry) {
        retryCounter++;
        me._debug('call [%s]: retry, count=%s', name, retryCounter);
        return start();
      }
    ...
    function start () {
      me._debug('call [%s]: start, timeout=%s', timeout);
